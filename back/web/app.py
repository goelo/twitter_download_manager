...
account_warmup_lock = threading.Lock()
...
ACCOUNT_WARMUP_STABLE_SUCCESS_THRESHOLD = max(1, int(os.environ.get('TW_ACCOUNT_WARMUP_STABLE_SUCCESS_THRESHOLD', '3') or 3))
ACCOUNT_WARMUP_BATCH_LIMIT = max(1, int(os.environ.get('TW_ACCOUNT_WARMUP_BATCH_LIMIT', '20') or 20))
ACCOUNT_WARMUP_ACTIVE_STATUSES = {'queued', 'running'}
...
def migration_baseline_schema(conn):
    ...
    ensure_column(conn, 'accounts', 'warmup_success_streak', 'integer not null default 0')
    ensure_column(conn, 'accounts', 'last_warmup_at', 'text')
    create_account_warmup_runs_table(conn)
...
def create_account_warmup_runs_table(conn):
    conn.executescript(
        ...
        create index if not exists idx_account_warmup_runs_status on account_warmup_runs(status, id);
        create index if not exists idx_account_warmup_runs_account on account_warmup_runs(account_id, id);
        '''
    )
...
def account_payload(account, capacity=None, conn=None):
    ...
    latest_warmup = None
    if conn:
        latest_warmup = account_warmup_latest_for_account(conn, account['id'])
    return {
        'id': account['id'],
        'label': account['label'],
        ...
        'warmup_success_streak': account['warmup_success_streak'] if 'warmup_success_streak' in account.keys() else 0,
        'last_warmup_at': account['last_warmup_at'] if 'last_warmup_at' in account.keys() else None,
        'latest_warmup': account_warmup_run_payload(latest_warmup) if latest_warmup else None,
        'capacity': capacity,
...
def account_warmup_run_payload(row):
    try:
        details = json.loads(row['details_json'] or '{}')
    except Exception:
        details = {}
    total = int(row['total'] if 'total' in row.keys() else 1) or 1
    done = int(row['done'] if 'done' in row.keys() else 0)
    return {
        'id': row['id'],
        ...
        'total': total,
        'done': done,
        'ok': int(row['ok'] if 'ok' in row.keys() else 0),
        'failed': int(row['failed'] if 'failed' in row.keys() else 0),
        'progress': min(100, int(done * 100 / total)) if total else 0,
...
def warmup_run_update(run_id, **fields):
    if not fields:
        return
    fields['updated_at'] = now()
    assignments = ', '.join(f'{key} = ?' for key in fields)
    with db() as conn:
        conn.execute(f'update account_warmup_runs set {assignments} where id = ?', (*fields.values(), run_id))


def account_warmup_latest_for_account(conn, account_id):
    return conn.execute(
        'select * from account_warmup_runs where account_id = ? order by id desc limit 1',
        (account_id,),
    ).fetchone()


def account_warmup_status_payload():
    with db() as conn:
        active = conn.execute(
            f"select * from account_warmup_runs where status in ({','.join('?' for _ in ACCOUNT_WARMUP_ACTIVE_STATUSES)}) order by id desc limit 1",
            tuple(ACCOUNT_WARMUP_ACTIVE_STATUSES),
        ).fetchone()
        latest = conn.execute('select * from account_warmup_runs order by id desc limit 1').fetchone()
    return {
        'active': account_warmup_run_payload(active) if active else None,
        'latest': account_warmup_run_payload(latest) if latest else None,
    }


def account_warmup_target_ids(conn, account_id=0):
    if account_id:
    ...
    rows = conn.execute(
        ...
        order by
          case when tier = 'new' then 0 else 1 end,
          coalesce(warmup_success_streak, 0) asc,
          id desc
        ''',
    ...
    for account in rows:
        ...
        streak = int(account['warmup_success_streak'] if 'warmup_success_streak' in account.keys() else 0)
        if tier == 'new' or capacity.get('score', 100) < 70 or (tier != 'stable' and streak < ACCOUNT_WARMUP_STABLE_SUCCESS_THRESHOLD):
            targets.append(account['id'])
        if len(targets) >= ACCOUNT_WARMUP_BATCH_LIMIT:
            break
    return targets


def create_account_warmup_run(account_ids, scope='single'):
    current_time = now()
    with db() as conn:
        active = conn.execute(
            f"select id from account_warmup_runs where status in ({','.join('?' for _ in ACCOUNT_WARMUP_ACTIVE_STATUSES)}) limit 1",
            tuple(ACCOUNT_WARMUP_ACTIVE_STATUSES),
        ).fetchone()
        if active:
            raise HTTPException(status_code=409, detail='已有账号养护正在运行，请完成后再启动。')
        cursor = conn.execute(
            '''
            insert into account_warmup_runs
              (account_id, status, scope, total, done, ok, failed, message, details_json, started_at, created_at, updated_at)
            ...
            (
                ...
                len(account_ids),
                '等待开始账号养护',
                json.dumps({'account_ids': account_ids}, ensure_ascii=False),
                current_time,
                current_time,
            ),
        )
...
def warmup_proxy_check(conn, account):
    proxy_note = '未绑定代理，按自动代理策略运行'
    proxy_ok = True
    proxy_id = account['bound_proxy_id'] if 'bound_proxy_id' in account.keys() else None
    if proxy_id:
        proxy = conn.execute('select * from proxies where id = ?', (proxy_id,)).fetchone()
        if proxy and proxy['enabled'] and proxy['status'] == 'active' and proxy_available_for_task(proxy):
            proxy_note = f'绑定代理可用：{proxy["label"]}'
        elif proxy:
            proxy_ok = False
            proxy_note = f'绑定代理暂不可用，将按自动代理回退：{proxy["label"]}'
        else:
            proxy_ok = False
            proxy_note = '绑定代理不存在，将按自动代理回退'
    return proxy_ok, proxy_note


def run_account_warmup_once(conn, account_id):
    account = conn.execute('select * from accounts where id = ?', (account_id,)).fetchone()
    if not account:
        return {'account_id': account_id, 'ok': False, 'message': '账号不存在'}
    capacity_before = account_capacity_payload(account, conn)
    if capacity_before.get('level') == 'cooldown':
        return {'account_id': account_id, 'ok': False, 'message': '账号正在冷却，跳过养护', 'capacity': capacity_before}
    proxy_ok, proxy_note = warmup_proxy_check(conn, account)
    refreshed, ok, error = check_account_row(account)
    with db() as update_conn:
        refreshed = update_conn.execute('select * from accounts where id = ?', (account_id,)).fetchone()
        current_streak = int(refreshed['warmup_success_streak'] if 'warmup_success_streak' in refreshed.keys() else 0)
        next_streak = current_streak + 1 if ok else 0
        tier = 'stable' if ok and next_streak >= ACCOUNT_WARMUP_STABLE_SUCCESS_THRESHOLD else account_tier(refreshed)
        update_conn.execute(
            '''
            update accounts
            set warmup_success_streak = ?, last_warmup_at = ?, tier = ?
            where id = ?
            ''',
            (next_streak, now(), tier, account_id),
        )
        refreshed = update_conn.execute('select * from accounts where id = ?', (account_id,)).fetchone()
        capacity_after = account_capacity_payload(refreshed, update_conn)
    message = '养护检测通过' if ok else f'养护检测未通过：{redact_sensitive(error)}'
    if not proxy_ok:
        message = f'{message}；{proxy_note}'
    return {
        ...
        'warmup_success_streak': int(refreshed['warmup_success_streak'] if 'warmup_success_streak' in refreshed.keys() else 0),
        'tier': refreshed['tier'] if 'tier' in refreshed.keys() else 'new',
        'capacity': capacity_after,
    }


def run_account_warmup(run_id, account_ids):
    warmup_run_update(run_id, status='running', started_at=now(), message='正在执行账号养护')
    results = []
    ok_count = 0
    failed_count = 0
    try:
        for index, account_id in enumerate(account_ids, start=1):
            warmup_run_update(run_id, message=f'正在养护账号 #{account_id}', done=index - 1)
            with db() as conn:
                result = run_account_warmup_once(conn, account_id)
            results.append(result)
            if result.get('ok'):
                ok_count += 1
                append_operation_log('info', 'account_warmup_ok', result.get('message') or '账号养护检测通过', details={'account_id': account_id})
            else:
                failed_count += 1
                append_operation_log('warning', 'account_warmup_failed', result.get('message') or '账号养护检测未通过', error_type='account_warmup_failed', details={'account_id': account_id})
            warmup_run_update(
                run_id,
                done=index,
                ok=ok_count,
                failed=failed_count,
                message=result.get('message') or f'已完成账号 #{account_id}',
                details_json=json.dumps({'account_ids': account_ids, 'results': results}, ensure_ascii=False, default=str),
            )
        status = 'completed' if failed_count == 0 else 'partial_failed'
        message = f'账号养护完成：成功 {ok_count} 个，需关注 {failed_count} 个'
        warmup_run_update(run_id, status=status, finished_at=now(), message=message)
    except Exception as exc:
        warmup_run_update(
            run_id,
            status='failed',
        ...
        append_operation_log('error', 'account_warmup_error', f'账号养护异常: {exc}', error_type='account_warmup_error')


def start_account_warmup(account_ids, scope='single'):
    if not account_ids:
        raise HTTPException(status_code=400, detail='没有需要养护的账号。')
    with account_warmup_lock:
        run_id = create_account_warmup_run(account_ids, scope)
        thread = threading.Thread(target=run_account_warmup, args=(run_id, account_ids), daemon=True, name=f'account-warmup-{run_id}')
        thread.start()
    with db() as conn:
        row = conn.execute('select * from account_warmup_runs where id = ?', (run_id,)).fetchone()
    return account_warmup_run_payload(row)
...
@app.get('/api/accounts/warmup/status')
def api_account_warmup_status(user=Depends(require_api_admin)):
    return account_warmup_status_payload()


@app.post('/api/accounts/warmup')
def api_warmup_accounts(user=Depends(require_api_admin)):
    with db() as conn:
        account_ids = account_warmup_target_ids(conn, 0)
    run = start_account_warmup(account_ids, scope='batch')
    return {'run': run}
...
def api_warmup_account(account_id: int, user=Depends(require_api_admin)):
    with db() as conn:
        account_ids = account_warmup_target_ids(conn, account_id)
    run = start_account_warmup(account_ids, scope='single')
    return {'run': run}
