...
访问 `http://127.0.0.1:8000` 后，首页会显示“X 舆情采集汇报看板”，包括任务总览、采集记录数、输出文件数、最近任务和演示模板。任务完成后，输出目录会生成 `summary_report.md`，用于快速说明采集目标、记录数、媒体数、互动指标和 Top 链接。

请注意：本项目适合内部研究、授权账号下的数据整理和原型演示。X/Twitter 存在官方 API、平台规则、速率限制、内容版权和隐私边界；生产化前建议迁移到官方 API，并单独确认授权、数据留存和合规要求。
